...
- Home replacement suppresses native Wi-Fi and single-subscription mobile presentation only when Combined Status can safely reproduce the represented state; unsupported or incomplete multi-subscription presentation remains SystemUI-owned and fails native rather than hiding information.
...
- Home Combined Status tint follows the live native status-icon tint authority for the non-charging battery ring, center network icon, and mobile layer; battery-applied tint remains only a bootstrap/fail-soft fallback when the primary Home tint source is unavailable. Charging ring state color remains independently owned, and the two user color-link controls can explicitly make center/mobile follow the final battery-ring color.
