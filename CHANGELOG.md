...
- Center and mobile-status tint now use SystemUI's own `DarkIconDispatcher.getTint(...)` evaluated against the verified 105 x 108 Home Battery/Combined Status visual anchor, rather than against the zero-width native `combined_status` shell or arbitrary peer icons. The native `ModernStatusBarViewBinding.onIconTintChanged(...)` value is retained as a fail-soft fallback, and the Battery applied tint remains the final fallback. Battery-ring state colors remain independently owned; the two user color-link controls still explicitly override center/mobile tint with the final battery-ring color when enabled.
