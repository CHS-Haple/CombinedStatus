...
- Diagnostic Build 300 withdraws Combined Status layout ownership of `MiuiStatusBatteryContainer.setIsHideBattery(Boolean)` while retaining the reversible `MiuiBatteryMeterView` child-alpha presentation mask. The native hide call is observe-only and all module geometry writes are zero, isolating whether the persistent battery-anchor layout mutation is responsible for the shade-edge right-shift regression.
