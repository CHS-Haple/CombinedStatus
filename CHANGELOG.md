...
- Native Wi-Fi, hotspot, and no-SIM center drawables use their visible alpha bounds to normalize optical size while retaining the HyperOS-authored drawable viewport as the centering anchor, preventing asymmetric resource ink from shifting the center off-axis while preserving native stroke design.
...
- Center and mobile-status tint now prefer the tint already applied to a live neighboring HyperOS status icon, then fall back to static icon color and dark-dispatcher tint-area resolution instead of reusing an early battery-only cache; battery-ring tint remains independently owned.
