...
- Home mobile replacement now distinguishes live active subscriptions from stale bound mobile roots using Android's active-subscription authority, with the existing SystemUI semantic state retained only as a fail-soft fallback; disabling one SIM therefore no longer turns an otherwise replaceable single-SIM presentation into a false dual-separate layout.
- Native Wi-Fi, hotspot, and no-SIM center drawables are sized and centered from their actual visible alpha bounds instead of their transparent intrinsic viewport, so HyperOS resources with different internal padding retain consistent optical size and alignment while preserving their native stroke design.
...
- Center and mobile-status tint now resolve from the active HyperOS status-icon dark-dispatcher authority (including tint-area evaluation) and refresh that authority instead of reusing an early battery-only cache, while battery-ring tint remains independently owned.
