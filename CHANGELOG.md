...
### Changed
- Appearance preview now separates bottom-navigation visual bounds from MIUIX system-safe-area semantics instead of compensating with full-scene crops. Both navigation modes share a fixed 74 dp visual viewport and the same final card height: standard NavigationBar uses its native 64 dp visual height plus 10 dp breathing room, while FloatingNavigationBar is measured at its native size, shifted 12 dp so its 52 dp capsule shares the same 64 dp visual baseline, and its internal 36 dp system-navigation tail is clipped by the preview viewport. The 82% preview scale and real MIUIX components remain unchanged.
