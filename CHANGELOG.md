...
- Native Wi-Fi, hotspot, and no-SIM center drawables use their visible alpha bounds to normalize optical size while retaining the HyperOS-authored drawable viewport as the centering anchor. The Wi-Fi family default visual box is enlarged while remaining governed by the shared center `sizeScale`, so normal, no-Internet, and hotspot variants scale together without changing their native stroke design.
