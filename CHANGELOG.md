...
- The color preview now follows MIUIX's read-only color-preview hierarchy: a section title, non-clickable preview card, current mode summary, and three compact real theme-token swatches with a uniform 1 dp low-alpha outline. The samples use primary, secondary, and primaryContainer to remain representative without resembling selectable buttons.
