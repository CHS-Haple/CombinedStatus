...
- Home native Combined Status suppresses replaceable Home Wi-Fi/mobile participants through the modern SystemUI binding availability and visibility-state contracts after handoff, removing both native slot allocation and rendered icon output while preserving the latest native visibility state for fallback; separate multi-root mobile presentation remains SystemUI-owned until Combined Status can represent every active SIM.
