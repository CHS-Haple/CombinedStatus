...
- Home native Combined Status now keeps native mobile suppression continuous across airplane-mode entry/exit and suppresses the Home `airplane` status-bar participant through its native `isIconVisible()` contract; the existing reversible `mobile_signal_container` alpha mask remains in place so externally injected signal descendants cannot flash during the HyperOS mobile-root recovery window, while separate dual-root mobile presentation still fails native.
