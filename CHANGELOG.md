...
- Center and mobile-status tint now consume the Combined Status participant's own native `ModernStatusBarViewBinding.onIconTintChanged(...)` callback, with the exact Home `MiuiBatteryMeterView` DarkReceiver retained only as fallback. Battery-ring state colors remain independently owned, and the two user color-link controls still explicitly override center/mobile tint with the final battery-ring color when enabled.
