...
### Changed
- Contributor engineering rules were reorganized around explicit MUST/SHOULD/MAY boundaries and now formalize runtime ownership, host-scoped sessions, disposal, single-writer discipline, geometry separation, fail-native fallback, reference-project limits, and the staged CombinedStatusModule ownership-migration gate.
- Default-data subscription selection now uses one event-driven, process-scoped source with an authoritative seed and change-only system broadcast refresh; Home rendering, native mobile presentation resolution, and connectivity sampling consume the cached value instead of repeatedly querying SubscriptionManager.
