...
### Changed
- Appearance preview now keeps one fixed 90 dp navigation viewport and one fixed 10 dp final crop for both navigation modes. Standard NavigationBar is bottom-aligned inside that shared viewport with crop compensation, removing the oversized lower tail without changing the preview/card height when the floating-navigation toggle changes. MIUIX components and the 82% preview scale remain unchanged.
- Appearance preview now crops the final rendered tail by navigation mode: floating navigation keeps a conservative 10 dp crop for shadow breathing room, while the standard navigation preview uses a 30 dp crop to remove its visibly oversized lower safe-area tail. The 90 dp navigation layout, 82% preview scale, and native MIUIX components remain unchanged.
- Appearance preview no longer trims bottom whitespace by changing the navigation scene's measured height. The CI138 90 dp navigation layout and 82% scale are preserved exactly; a 10 dp crop is applied only after the complete miniature scene has been laid out and scaled, so bottom-bar size and position remain unchanged while only the final rendered tail is removed.
...
- Appearance preview returns to CI138's 82% miniature-scene proportions and CI138 floating-navigation geometry. Only the preview navigation scene height is reduced from 90 dp to 64 dp, preserving the native 52 dp capsule and roughly 12 dp of bottom breathing room for its shadow instead of the full MIUIX system-navigation tail.
