...
- Home mobile replacement now distinguishes live active subscriptions from stale bound mobile roots using Android's active-subscription authority, with the existing SystemUI semantic state retained only as a fail-soft fallback; disabling one SIM therefore no longer turns an otherwise replaceable single-SIM presentation into a false dual-separate layout. Native mobile signal containers are also pre-masked at the verified emitter boundary while replacement suppression is already active, preventing a newly rebound SIM root from painting one transient native frame before the normal binding revalidation completes.
...
- Center and mobile-status tint now use the applied tint from the exact Home `MiuiBatteryMeterView` dark-receiver that anchors the Combined Status slot. Generic peer-icon sampling remains diagnostic-only and no longer overrides rendering, preventing masked or stale native network participants from supplying an unrelated tint. Battery-ring state colors remain independently owned.
