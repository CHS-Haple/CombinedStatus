...
- Native Home battery presentation now yields to Combined Status through HyperOS `MiuiStatusBatteryContainer.setIsHideBattery(Boolean)` for layout ownership plus a reversible `MiuiBatteryMeterView` invisible visual mask, preserving the attached battery state/tint/scene lifecycle and restoring both native island hide state and native visibility on fallback; ordinary and quick-charge child updates remain SystemUI-owned beneath the masked battery root.
