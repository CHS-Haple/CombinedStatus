...
- The verified SystemUI AirplaneModeRepository setIsAirplaneMode command is now the primary airplane-state trigger, committing CombinedStatus before repository work proceeds; mobile-signal sampling remains only a fallback.
...
- Home dynamic-island motion now follows SystemUI's own Home island-status lifecycle: during a native island transition, CombinedStatus temporarily re-resolves the real battery anchor at pre-draw time and stops after the native anchor stabilizes, preserving SystemUI timing/easing without a custom offset formula.
