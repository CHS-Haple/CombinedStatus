...
- Home native Combined Status suppresses replaceable Home Wi-Fi/mobile participants through the binding visibility contract after handoff; for the native mobile signal subtree, a reversible `mobile_signal_container` alpha mask prevents later descendant renderers from repainting signal content outside the collapsed slot without changing root visibility or geometry, while separate dual-root mobile presentation remains SystemUI-owned.
