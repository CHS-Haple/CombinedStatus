...
- Home native Combined Status suppresses replaceable Home Wi-Fi/mobile participants through the modern SystemUI view contract after handoff: `isIconVisible()` yields native slot participation and `setVisibleState(STATE_HIDDEN)` masks rendered output while the latest native visibility state is preserved for fallback; separate multi-root mobile presentation remains SystemUI-owned until Combined Status can represent every active SIM.
