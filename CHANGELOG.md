...
- Build 301 keeps the native battery slot and `setIsHideBattery(Boolean)` fully SystemUI-owned after Build 300 eliminated the shade-edge right-shift, while changing only the module-owned `combined_status` participant to a zero-width native slot whose 105px render surface overflows into the preserved battery slot. This removes the duplicate 105px steady-state allocation without writing peer battery geometry; handoff now verifies zero-slot alignment to the real battery screen X before committing.
