...
- Home `no_sim` replacement now consumes the native `StatusBarIconView.isIconVisible()` result in the same visibility event that HyperOS uses to present the icon, instead of depending on an earlier `onIconAdded`/policy snapshot. The native no-SIM drawable is resolved before suppression is enabled, so Combined Status can move it into the center and suppress only the matching Home icon in the same event; unresolved resources remain fail-native.
- Home mobile replacement distinguishes live active subscriptions from stale bound mobile roots using Android's active-subscription authority, with existing SystemUI semantic state retained only as a fail-soft fallback. Native mobile signal containers are pre-masked at the verified emitter boundary only when their ancestor chain belongs to the active Home status-icon group; Control Center / shade mobile participants remain untouched. An already-suppressed Home session stays suppressed across the narrow duplicate-root `DUAL_SEPARATE` rebind window where bound mobile roots temporarily outnumber active subscriptions; once topology settles, the normal multi-SIM fail-native policy applies without any timer or delayed release.
...
- Native Wi-Fi, hotspot, airplane, and no-SIM center drawables use visible alpha bounds to normalize optical size while retaining the HyperOS-authored drawable viewport as the centering anchor. Stable native center assets are rasterized directly into final View pixel bounds instead of inheriting the Combined Status canonical-canvas scale, and receive the full HyperOS ARGB tint in the same form as a native tinted ImageView. This keeps the accepted Wi-Fi visual size while avoiding a second resampling/alpha-composition step; transition frames retain the existing canonical animation contract.
...
- Home Combined Status tint uses the live native `ModernStatusBarViewBinding.onIconTintChanged(...)` callback as the sole status-icon authority once the native participant is active. The non-charging battery ring, center network icon, and mobile layer resolve from that callback; Battery applied tint is retained only as the bootstrap/fail-soft fallback for the pre-handoff overlay or missing native tint. The redundant Battery-slot `DarkIconDispatcher.getTint(...)` derivation is removed. Charging ring state color remains independently owned, and the two user color-link controls can still explicitly make center/mobile follow the final battery-ring color.
