...
- Native Home battery presentation now yields to Combined Status through HyperOS `MiuiStatusBatteryContainer.setIsHideBattery(Boolean)` for layout ownership plus a reversible alpha mask on `MiuiBatteryMeterView` presentation children; the real battery root remains visible and fully SystemUI-owned for alpha/translation/transition motion, while child drawable/charging updates continue natively beneath the mask and native child alpha is restored on fallback.
