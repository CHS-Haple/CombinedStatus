...
- Center and mobile-status tint now use the native `ModernStatusBarViewBinding.onIconTintChanged(...)` callback of the live Combined Status Home participant as the first authority. The verified 105 x 108 Battery/Combined Status visual-slot `DarkIconDispatcher.getTint(...)` result remains a fail-soft fallback, followed by the Battery applied tint. Battery-ring state colors remain independently owned; the two user color-link controls still explicitly override center/mobile tint with the final battery-ring color when enabled.
